...
export function getAuthMode(): AuthMode {
  const useCanfar =
    process.env.NEXT_USE_CANFAR === "true" ||
...
export function getOIDCConfig(allowMissing = false): OIDCConfig {
  ...
  const clientId =
    process.env.NEXT_OIDC_CLIENT_ID || process.env.NEXT_PUBLIC_OIDC_CLIENT_ID;
  const clientSecret = process.env.NEXT_OIDC_CLIENT_SECRET || "";
  const callbackUrl =
    process.env.NEXT_OIDC_CALLBACK_URI ||
    process.env.NEXT_PUBLIC_OIDC_CALLBACK_URI;
  const redirectUrl =
    process.env.NEXT_OIDC_REDIRECT_URI ||
    process.env.NEXT_PUBLIC_OIDC_REDIRECT_URI;
  const scope =
    process.env.NEXT_OIDC_SCOPE ||
...
export function validateAuthConfig(): { valid: boolean; errors: string[] } {
  ...
  if (mode === "OIDC") {
    try {
      ...
      errors.push(
        error instanceof Error ? error.message : "Invalid OIDC configuration",
      );
    }
  } else if (mode === "CANFAR") {
    ...
    if (!loginApi) {
      errors.push(
        "Missing LOGIN_API or NEXT_PUBLIC_LOGIN_API for CANFAR authentication",
      );
    }
  }
